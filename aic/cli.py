...
def main():
    ...
    # aic setup
    subparsers.add_parser("setup", help="One-time AI Core infrastructure setup")

    # aic deploy
    deploy_parser = subparsers.add_parser("deploy", help="Deploy a model to AI Core")
    deploy_parser.add_argument("--config", default=None, help="Path to deployment_config.yaml — skips all prompts")
    ...
    if args.command == "setup":
        ...
        if args.list:
            ...
            deploy.run(config_path=args.config, scenario_id=args.scenario)
